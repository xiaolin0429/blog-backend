...
class TestUserSerializer:
    ...
    def test_user_register_serializer_duplicate_username(
        self, test_user, valid_register_data
    ):
        """测试用户注册序列化器-用户名重复"""
        data = valid_register_data.copy()
    ...
    def test_user_profile_update_serializer_valid(self, test_user):
        """测试用户资料更新序列化器-有效数据"""
        update_data = {"nickname": "Updated User", "bio": "Updated bio"}
        serializer = UserProfileUpdateSerializer(
            test_user, data=update_data, partial=True
        )
        assert serializer.is_valid()
