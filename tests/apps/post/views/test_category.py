...
class TestCategoryViews:
    ...
    def test_search_categories(self, auth_client, parent_category):
        ...
        response = auth_client.get(
            reverse('post:category_list'),
            {'search': parent_category.name}
        )
        assert response.status_code == status.HTTP_200_OK
        assert response.data['code'] == 200
        assert isinstance(response.data['data'], list)
        assert len(response.data['data']) > 0
        assert any(category['name'] == parent_category.name for category in response.data['data'])
    ...
    def test_quick_create_category_duplicate_name(self, auth_client, category):
        ...
        response = auth_client.post(
            reverse('post:category_quick_create'),
            {'name': category.name}
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_nonexistent_category(self, auth_client):
        ...
        data = {'name': 'Updated Category'}
        response = auth_client.put(
            reverse('post:category_detail', args=[999]),
            data
        )
        assert response.status_code == status.HTTP_200_OK
